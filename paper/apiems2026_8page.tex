...
\title{\vspace{-8mm}\textbf{Compound-Truck Cross-Docking Scheduling with Release Times and Soft Due Dates:\\A Bottleneck-Guided Iterated Local Search}}
\author{Youngsoo Kim\\
...
learned operator selection does not outperform uniform sampling at an equal number of iterations.
\end{abstract}
...
Those studies keep trucks purely inbound or outbound. The compound-truck
setting is different: a truck is partially unloaded, retains one destination,
waits for the incoming transfers feeding it, and only then departs as a carrier.
This partial-unloading transformation induces a precedence chain
(arrival $\to$ partial unload $\to$ retained cargo $\to$ transfer wait $\to$
outbound departure), so a late arrival propagates to every dependent carrier and
a soft due date turns the cascade into a tardiness cost. Our contribution is to
formulate and study this interaction of \emph{known} release times and soft due
dates with the partial-unloading carrier transformation.
...
not improve on a uniform control at an equal number of iterations in our experiments.
...
for the preceding occupant of its door; it begins door service at that time and
then docks, loads, and undocks (so loading starts only after docking $DE_f$).
Loading time for destination $d$ is
...
the per-instance gap $\Delta_{bo}$ to the best solution observed across all
methods, iteration counts, and CP-SAT where available.
...
\begin{table*}[t]
\centering
\caption{Mean test objective and gap to the best observed solution; only S-none
is proven optimal.}
\label{tab:quality}
\small
...
Size & VAA & GILS & $\Delta_{bo}$ & VAA & GILS & $\Delta_{bo}$ & VAA & GILS & $\Delta_{bo}$ \\
\midrule
...
Where SA-RL5 is defined, the ordering GILS $<$ SA-RL5 $<$ VAA holds. Using the
default uniform selector, paired tests show that GILS improves on VAA by
2.65\% on average ($n=180$, $p<10^{-4}$) and on SA-RL5 by 1.38\%
($n=60$, $p<10^{-4}$); tabular-Q and DQN selection are examined in the ablation.
...
\caption{Mean gap to the best observed solution by operator pool.}
\label{fig:operators}
\end{figure}
...
selector wins consistently over iteration counts from 50 to 3,000. This negative
...
\caption{Mean gap to the best observed solution versus the number of iterations. The tabular selector
only edges uniform at the largest iteration count, while transfer DQN never wins.}
...
instance, and then average. Figure~\ref{fig:lambda} shows an empirical trade-off.
...
present, a uniform selector is as effective as the learned alternatives at an equal number of iterations. SA acceptance and reheating are retained for comparability with
the base SA-RL5 framework, although the ablation indicates they are not necessary
for solution quality in the present setting. This does not imply that learning is
...
During preparation, the author used OpenAI Codex (GPT-5) and Claude Code
(Anthropic) for language editing, manuscript organization, bibliographic
checking, and assistance in developing and reviewing computational code. The
author reviewed and verified all references, formulations, code, results, and
interpretations, and takes full responsibility for the content.

\begin{thebibliography}{99}\small
\bibitem{assadi2016} M. T. Assadi and M. Bagheri, ``Differential evolution and
population-based simulated annealing for truck scheduling problem in multiple door cross-docking systems,'' \emph{Computers
...
scheduling and assignment of trucks at unit-load cross-dock terminals with mixed service mode dock doors,''
...
robust optimization models for cross-dock truck scheduling problem under uncertainty,''
