...
(soft due date)을 도입하고, makespan과 총 납기지연(tardiness)의 합을 최소화하는 문제로 확장한다. 문제를 mixed-integer
...
release time 및 연성 납기시각이 부분 하역 후 출고 운송 트럭으로 전환되는 과정과 결합되는 방식 --- 을
정식화하고 분석하는 것이다.
...
각 목적지에는 정확히 하나의 운송 트럭이 배정된다. 트럭 $q\in I\cup F$는 release time
$r_q$ 이전에 시작할 수 없고 연성 납기시각 $\bar d_q$를 갖는다. 완료 시각을 $C_q$,
