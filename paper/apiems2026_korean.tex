...
\begin{abstract}
\noindent
입고와 출고를 병행하는 트럭(compound truck, 이하 병행 트럭)이 부분 하역된 뒤 특정 목적지의
출고 운송 트럭으로 전환되는 다중 도어 크로스도킹 스케줄링 문제를 다룬다.
기존 병행 트럭 모델은 모든 트럭이 시각 0에 도착한다고 가정하고 총 작업완료시간
...
operator selection(tabular Q-learning, transfer DQN)은 동일한 반복 횟수에서 uniform
sampling을 능가하지 못하였다.
...
재고와 리드타임을 줄이는 물류 방식이다~\cite{boysen2010,vanbelle2012}. 병행 트럭 변형에서는 한 대의 트럭이 두 역할을 겸한다. 다른 목적지의 화물을 하역하되
한 목적지의 화물은 유지하고, 이후 해당 목적지의 화물을 다른 병행 트럭들로부터 받아
...
다룬다. 그러나 이들은 트럭을 순수 입고 또는 출고 트럭으로 고정한다. 병행 트럭 설정은 다르다. 트럭은 부분 하역 후 한 목적지의 화물을 유지하고, 해당 목적지로
...
비롯되는지를 구분한다. 후자는 동일한 반복 횟수의 균등 선택 방식을 능가하지 못하였다.
...
$h_{id}=\sum_k f_{idk}t_k$, $H_i=\sum_d h_{id}$로 둔다. 각 병행 트럭은 유지할
목적지 하나와 도어 하나를 고르며 도어당 병행 트럭 최대 1대이다. 각 출고 트럭은
남은 목적지 하나와 도어를 고르고, 같은 도어의 출고 트럭은 작업 순서를 정한다. 따라서
각 목적지에는 정확히 하나의 운송 트럭이 배정된다. 트럭 $q\in I\cup F$의 release
time을 $r_q$로 나타낸다. 따라서 $r_i$는 병행 트럭 $i\in I$가 도어 작업을
...
시간 계산은 물리적 작업 순서를 따른다. 병행 트럭 $i$가 목적지 $d$의 화물을 유지하면 하역은
...
\paragraph{MILP 요약.} 이진 변수 $x_{idm}$은 병행 트럭 $i$를 유지 목적지
$d$·도어 $m$에, $z_{fdm}$은 출고 트럭 $f$를 배정한다. 운송 트럭 및 병행 트럭의
...
\textbf{Descent.} 세 무브군(출고 트럭 재배치, 병행 트럭의 도어 교환/빈 도어 이동, 두
...
tabular-Q/transfer-DQN selection의 GILS이다. 주 지표는 모든 방법과 반복 횟수 및 CP-SAT를
...
열세이다($p<10^{-4}$)~\cite{watkins1992,mnih2015}. 반복 횟수를 50--3{,}000회로 바꾸어도 어떤 학습 기반 선택기도 일관되게 앞서지 못하며, 이러한 부정적 결과는 무시간창 셀에서도
성립하므로 시간창 확장 때문에 나타난 결과가 아니다(그림~\ref{fig:selector}).

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{fig1_selector_budget.pdf}
\caption{반복 횟수에 따른 관측된 최선해 대비 gap. tabular는 최대
반복 횟수에서만 uniform보다 근소하게 앞서고, transfer DQN은 이기지 못한다.}
\label{fig:selector}
\end{figure}
...
알려진 release time과 soft due date는 병행 트럭 스케줄을 실질적으로 바꾼다.
...
탐색을 이끈다. 이 구조가 갖춰지면 uniform selector가 동일한 반복 횟수에서 learned
