...
\begin{abstract}
\noindent
컴파운드 트럭(compound truck)이 부분 하역된 뒤 특정 목적지의 출고 트럭으로
전환되는 다중 도어 크로스도킹 스케줄링 문제를 다룬다. 기존 컴파운드 트럭 모델은
...
선택은 동일 예산의 균등 선택 대비 추가 이득을 보이지 않았다.
...
반면 본 문제의 유연성은 트럭 수준에 있어, 부분 하역된 컴파운드 트럭이 특정 목적지의
...
\textbf{초기해 구성.} 원 모델의 VAA는 Vogel식 regret으로 컴파운드 트럭의 유지 목적지를
...
\caption{20-인스턴스 test 결과(실험 조건당 인스턴스 20개 $\times$ 반복 5회).
...
\begin{table}[H]\centering
\caption{Operator-pool ablation(실험 조건당 인스턴스 5개 $\times$ 반복 5회, 1{,}000회
예산 고정; 양측 Wilcoxon, $+$평균 = 더 풍부한 연산자 집합이 우세).}
\label{tab:b1}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
대비 & $n$ & 개선(\%p) & $p$\\
...
\begin{table}[H]\centering
\caption{Engine-component ablation(실험 조건당 인스턴스 5개 $\times$ 반복 5회;
...
\begin{table}[H]\centering
\caption{selection 효과(실험 조건당 인스턴스 5개 $\times$ 반복 5회, 1{,}000회 예산;
...
\caption{iteration budget별 관측된 최선해 대비 gap. 50--3{,}000회에서 uniform은
$0.47\to0.16\%$, tabular는 $0.56\to0.09\%$, DQN은 $0.56\to0.26\%$로 움직이며,
어떤 예산에서도 learned selector가 uniform을 뚜렷하게 능가하지 않는다.}\label{fig:sel}
\end{figure}
