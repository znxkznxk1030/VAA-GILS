...
20-인스턴스 기준이고, vs CP-SAT는 CP-SAT 기준해가 있는 부분집합(S 조건 5개, M-none 2개)만이며
S-none만 최적이 증명된다. CP-SAT 기준해가 있는 모든 실험 조건에서 GILS는 이에 근접한다(기본 selector인
uniform 기준 0.11--0.21\%, 학습 selector를 포함한 전체 변형 기준 0.1--0.6\%). 기본(default) selector인 uniform 기준으로, GILS는 VAA를
...
\begin{table}[H]\centering
\caption{Operator-pool ablation(양측 Wilcoxon; $+$평균 = 더 풍부한 연산자 집합이 우세).}
\label{tab:b1}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
대비 & $n$ & 개선(\%p) & $p$\\
...
있다. 이는 도착 정보가 시간의 흐름에 따라 공개되는 online dock control이나 개별 이동의 평가 비용이
