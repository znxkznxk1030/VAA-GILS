...
\caption{Processing timeline of a compound truck and the outbound truck of its
retained destination.}\label{fig:timeline}
\end{figure}
...
항목 & 기호 & 값 \\
\midrule
규모(병행/출고/목적지/도어) & $(|I|,|F|,|D|,|M|)$ & S $(6,3,9,6)$ · M $(12,6,18,12)$ · L $(20,10,30,20)$ \\
병행 트럭 비율 & $|I|/|D|$ & $2/3$ \\
상품 종류 수 & $|K|$ & 3 \\
화물량 & $f_{idk}$ & $U[0,20]$ \\
단위 처리시간 & $t_k$ & $U[1,4]$ \\
진입·출차 시간 & $DE_q,DL_q$ & $U[1,5]$ \\
도어 좌표 & --- & $[0,100]^2$ 균등 \\
도어 간 이송시간 & $t_{mn}$ & 유클리드 거리 $/\,10$ \\
도착 시각 & $r_q$ & none $0$ / 그 외 $U[0,\rho H]$ \\
soft due date & $\bar d_q$ & none $\infty$ / 그 외 $r_q+\delta H$ \\
시간 제약 수준 & $(\rho,\delta)$ & medium $(0.25,0.60)$ · tight $(0.50,0.35)$ \\
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize 화물량·처리시간·진입·출차 시간은 정수 균등분포이다.}
\end{table}
