...
\begin{abstract}
\noindent
입고와 출고를 병행하는 트럭(compound truck, 이하 병행 트럭)이 부분 하역된 뒤 특정 목적지의 출고트럭으로
...
soft due date를 도입하고, 총 작업완료시간과 총 납기지연(Tardiness)의 합을 최소화하는
문제로 확장한다. 문제를 mixed-integer program과 CP-SAT 모형으로 정식화하고, 서로
겹치지 않는 학습·조정·시험 seed 집합을 사용한 재현 가능한 실험 프로토콜을
...
\smallskip
\noindent\textbf{주제어:} cross-docking; compound truck; 도착 시각; soft due date;
...
납기지연을, Assadi와 Bagheri~\cite{assadi2016}는 ready time과 outbound
조기완료와 납기지연(earliness/납기지연)을, Bodnar 등~\cite{bodnar2017}은 time window와 mixed-service
...
완전·부분·부재(full/partial/absent) 정보를 구분하고, Ladier와 Alpan~\cite{ladier2016}은 time window
...
\caption{Structural comparison of the base SA-RL5 and VAA-GILS.}\label{tab:base}\small
...
연산자 선택 & online Q-learning/roulette & 균등 무작위 (학습은 ablation 전용) \\
...
목적식 & 총 작업완료시간 & 총 작업완료시간 $+\lambda\cdot$ 납기지연 \\
\bottomrule
\end{tabular}
\end{table}
...
상품은 3종이며 트럭 $q\in I\cup F$에 대해 $f_{idk}\sim U[0,20]$, $t_k\sim U[1,4]$,
$DE_q,DL_q\sim U[1,5]$로 모두 정수 균등분포에서 뽑는다. 화물이 특정 목적지로 쏠리는
정도는 이와 별개의 조건이며, 보고하는 모든 실험은 쏠림이 없는 조건을 사용한다(쏠린
조건과 군집 조건은 부록 A의 transfer DQN 학습에만 쓴다). 도어는 $[0,100]^2$에 균등 랜덤
좌표로 배치하고 도어 간 이송시간 $t_{mn}$은 두 좌표 사이 유클리드 거리를 10으로 나눈
값이다. 도어를 일렬로 늘어놓으면 이송시간이 도어 번호 차이로 정해져 도어 배정이 해에
미치는 영향이 작아지므로, 배정 결정이 실제로 품질을 가르도록 무작위 배치를 쓴다. 거리를
10으로 나누는 것은 이송시간을 처리시간과 같은 크기 범위에 두기 위해서다.
...
\caption{Test results over 20 instances per experimental condition (20 instances $\times$ 5 replications). $\Delta_{bo}$ is the mean gap to the best observed solution and time is the mean per run. The vs CP-SAT column is computed only on the subset for which CP-SAT returned a reference (5 instances per S condition, 2 for M-none); optimality was proven only for S-none. On that subset the mean CP-SAT runtime is 76 s (S) and 237 s (M-none).}\label{tab:main}\small
...
\begin{table}[H]\centering
\caption{Operator-pool ablation (5 instances $\times$ 5 replications per condition, the number of iterations fixed at 1,000; two-sided Wilcoxon, positive mean favors the richer operator set).}\label{tab:b1}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
대비 & $n$ & 개선(\%p) & $p$\\
\midrule
기본 $\to$ 핵심 (g1,g2 추가) & 136 & $+0.114$ & $<10^{-4}$\\
핵심 $\to$ 전체 (g3,g4 추가) & 172 & $+0.048$ & $<10^{-4}$\\
기본 $\to$ 전체 (유도 전부) & 166 & $+0.161$ & $<10^{-4}$\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]\centering
\caption{Engine-component ablation (5 instances $\times$ 5 replications per condition; leave-one-out, relative objective degradation when a component is removed).}\label{tab:b2}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
제거 구성요소 & $n$ & 악화(\%p) & $p$\\
\midrule
...
반복 횟수에 따라 뒤집히며(부록 <Figure~\ref{fig:sel}>),
이러한 부정적 결과는 무제약 조건만으로도 성립하므로 시간 제약 확장 때문에 나타난
결과가 아니다.

\begin{table}[H]\centering
\caption{Effect of the selection policy (5 instances $\times$ 5 replications per condition, 1,000 iterations; two-sided Wilcoxon, positive mean favors the first method).}\label{tab:sel}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
비교 & 평균차(\%p) & $p$ & 판정\\
\midrule
...
식~\eqref{eq:obj}이 시간 단위 성분 둘을 합치므로 조정 집합에서
...
\caption{Mean gap to the best observed solution versus the number of iterations.}\label{fig:sel}
\end{figure}
