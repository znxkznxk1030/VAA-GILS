...
\begin{abstract}
\noindent
입고와 출고를 병행하는 트럭(compound truck, 이하 병행 트럭)이 부분 하역된 뒤 특정 목적지의 출고 트럭으로
전환되는 다중 도어 크로스도킹 스케줄링 문제를 다룬다. 기존 병행 트럭 모델은
모든 트럭이 시각 0에 도착한다고 가정하고 총 작업완료시간(makespan)만 최소화한다.
본 연구는 트럭별 도착 시각(release time)과, 위반을 허용하되 지연 비용을 부과하는
soft due date를 도입하고, makespan과 총 납기지연(tardiness)의 합을 최소화하는
문제로 확장한다. 문제를 mixed-integer program과 CP-SAT 모형으로 정식화하고, 서로
겹치지 않는 train, tuning, test seed 집합을 사용한 재현 가능한 실험 프로토콜을
제시한다. 제안 방법인 VAA-GILS는 Vogel식 구성 기법으로 생성한 초기해에
best-improvement descent, 병목(bottleneck) 유도 연산자, 그리고 kick restart를
결합한 iterated local search이다. CP-SAT가 기준해를 제공한 실험 조건에서
VAA-GILS는 그 기준해 대비 평균 0.11--0.21\%의 목적값을 1초 미만에 얻었다(CP-SAT는
76--237초). 이 가운데 소형 무제약 조건은 CP-SAT가 최적성을 증명한 경우로, 제안
방법은 증명된 최적해 대비 평균 0.21\%였다. CP-SAT가 600초 이내에 실행 가능한 해를
찾지 못한 대형 시간 제약 조건에서도, 제안 방법은 비교 방법 가운데 가장 우수한 해를
제공하였다. Ablation 실험 결과, 성능 향상에는 descent, 병목 유도 연산자, restart로
구성된 결정론적 탐색 구조가 주요하게 기여한 것으로 나타났다. 한편 학습 기반 연산자
선택은 동일한 반복 횟수의 균등 선택 대비 추가 이득을 보이지 않았다.
...
발전시킨 병행 트럭 변형에서는 한 대의 트럭이 두 역할을 겸한다. 여러 목적지
...
\subsection{기본 모델}
$I$, $F$, $D$, $K$, $M$을 각각 병행 트럭, 출고 트럭, 목적지, 상품 종류,
...
병행 트럭의 도어 작업 완료, 출고 도어 작업 시작, 출고 완료, makespan이다.
$L_{id}=\sum_{j\ne i}h_{jd}$, $L_d=\sum_i h_{id}$, $B$는 유효한 horizon 상한이다.

배정 제약은 다음과 같다.
\begin{equation}\textstyle\sum_{d,m}x_{idm}=1\quad\forall i\tag{1}\end{equation}
\begin{equation}\textstyle\sum_{d,m}z_{fdm}=1\quad\forall f\tag{2}\end{equation}
\begin{equation}\textstyle\sum_{i,m}x_{idm}+\sum_{f,m}z_{fdm}=1\quad\forall d\tag{3}\end{equation}
\begin{equation}\textstyle\sum_{i,d}x_{idm}\le1\quad\forall m\tag{4}\end{equation}

병행 트럭의 시간 관계와 유입 이송 선행 제약은 다음과 같다.
\begin{equation}U_i=r_i+DE_i+H_i-\textstyle\sum_{d,m}h_{id}x_{idm}\quad\forall i\tag{5}\end{equation}
\begin{equation}C_i\ge U_i+L_{id}+DL_i-B(1-x_{idm})\quad\forall i,d,m\tag{6}\end{equation}
\begin{equation}C_i\ge U_j+t_{nm}+L_{id}+DL_i-B(2-x_{idm}-X_{jn})\tag{7}\end{equation}
식~(7)은 모든 $i,d,m,n$과 $\sum_k f_{jdk}>0$인 기여 트럭 $j\ne i$에 대해 성립한다.

출고 운송 트럭에 대해서는 다음이 성립한다.
\begin{equation}S_f\ge r_f\quad\forall f\tag{8}\end{equation}
\begin{equation}C_f\ge S_f+DE_f+L_d+DL_f-B(1-Z_{fd})\quad\forall f,d\tag{9}\end{equation}
\begin{equation}S_f\ge U_i+t_{nm}-B(2-z_{fdm}-X_{in})\tag{10}\end{equation}
\begin{equation}S_f\ge C_i-B(2-Z_{fm}-X_{im})\quad\forall f,i,m\tag{11}\end{equation}

같은 도어 $m$을 쓰는 출고 트럭 쌍 $f<g$에 대한 순서 결정과 non-overlap 제약은
다음과 같다.
\begin{equation}b_{fgm}+b_{gfm}\ge Z_{fm}+Z_{gm}-1,\quad b_{fgm}\le Z_{fm},\quad b_{fgm}\le Z_{gm}\tag{12}\end{equation}
\begin{equation}S_g\ge C_f-B(1-b_{fgm})\tag{13}\end{equation}
\begin{equation}S_f\ge C_g-B(1-b_{gfm})\tag{14}\end{equation}
끝으로 makespan은 다음으로 정의된다.
\begin{equation}C_{\max}\ge C_i\quad\forall i,\qquad C_{\max}\ge C_f\quad\forall f\tag{15}\end{equation}
...
\subsection{시간 제약 확장과 목적함수}
각 트럭 $q\in I\cup F$는 앞서 정의한 release time $r_q\ge0$과 soft due date $\bar d_q$를 갖고
$T_q\ge\max\{0,C_q-\bar d_q\}$이다. 목적함수는
\begin{equation}\min\ J=C_{\max}+\lambda\sum_{q\in I\cup F}T_q,\qquad \lambda=1
\label{eq:obj}\tag{16}\end{equation}
...
사용한다. 병행 트럭 $i$(목적지 $d$ 유지)의 최속 완료와 출고 트럭 $f$의 최속
완료는
\begin{equation}EF_i=r_i+DE_i+\min_{d\in D}\Big(H_i-h_{id}+\textstyle\sum_{j\in I\setminus\{i\}}h_{jd}\Big)+DL_i\tag{17}\end{equation}
\begin{equation}EF_f=r_f+DE_f+\min_{d\in D}\textstyle\sum_{j\in I}h_{jd}+DL_f\tag{18}\end{equation}
critical-chain 하한은 $LB_{\mathrm{cc}}=\max_{q\in I\cup F}EF_q$이다. door-area
하한은 각 트럭의 최소 도어 점유
\begin{equation}o_i=DE_i+\Big(H_i-\max_d h_{id}\Big)+\min_d\textstyle\sum_{j\in I\setminus\{i\}}h_{jd}+DL_i\tag{19}\end{equation}
\begin{equation}o_f=DE_f+\min_d\textstyle\sum_{j\in I}h_{jd}+DL_f\tag{20}\end{equation}
을 도어 수로 나눈 값이다.
\begin{equation}LB_{\mathrm{da}}=\frac{1}{|M|}\Big(\sum_i o_i+\sum_f o_f\Big)\tag{21}\end{equation}
두 하한 모두 제약을 완화하여 얻으므로 이들의 최댓값
$LB_\tau=\max\{LB_{\mathrm{cc}},LB_{\mathrm{da}}\}$는 makespan의 유효 하한이다.
또한 각 $EF_q$가 $C_q$의 하한이므로 구조적 지연 하한은
\begin{equation}LB_T=\sum_{q:\bar d_q<\infty}\max\{0,\ EF_q-\bar d_q\}\tag{22}\end{equation}
이며, $LB_J=LB_\tau+\lambda LB_T$는 식~\eqref{eq:obj}의 유효 하한이다.

\section{VAA-GILS}

VAA-GILS는 iterated local search의 구현체로, 연산자 내부 sampling과 acceptance를
제외하면 결정론적이다. 이하에서 \textbf{원 모델}은 Shahmardan과
Sajadieh~\cite{shahmardan2020}가 제안한 SA-RL5를 가리키며, VAA 구성 휴리스틱과
generic 이웃 7종은 이 모델에서 가져왔다.
...
이른 도어에 차례로 삽입한다. 모든 완료 예정 시각 계산에는 release time을 반영한다.

\textbf{Descent.} 세 가지 이동 유형(출고 트럭 재배치, 병행 트럭의 도어 교환/빈 도어 이동, 두
...
수행한다. 현재 최선해 $s^*$의 복사본에서 다시 시작하되, 아래 generic 이웃 7종에서
...
(k1) 두 트럭의 목적지 교환, (k2) 병행 트럭 간 도어 교환, (k3) 출고 트럭 간 도어
교환, (k4) 출고 트럭을 다른 도어로 삽입, (k6) 하역시간 기반 룰렛 선택으로 병행 트럭을 도어에 삽입, (k7) 적재시간 기반 룰렛 선택으로 출고 트럭을 도어에 삽입,
...
강화할 뿐이므로 비교 대상 selector를 불리하게 만들지 않는다. 원 모델 SA-RL5는 RL이 단일 이동을 고르고 SA accept/reject로 한 단계가 끝나는
...
\section{실험 설계}
세 규모 $(|I|,|D|,|M|)$: S $(6,9,6)$, M $(12,18,12)$, L $(20,30,20)$, 병행 트럭
...
동일한 반복 횟수·동일 구조 분해이므로 원래의 5-인스턴스 설계를 유지한다. 인스턴스별로
replication을 평균한 뒤 짝지은 인스턴스 평균에 양측 Wilcoxon signed-rank test를
적용한다. GILS의 종료 조건은 반복 횟수 1{,}000회다. CP-SAT는 인스턴스당 1회, 8 thread로 S
300초(5개), M/L 600초(조건당 2개) 실행하며 S-none 5개만 최적이 증명된다. 주 지표는
모든 방법과 반복 횟수 및 CP-SAT 결과를 포함하여 관측된 최선해에 대한 인스턴스별 gap
$\Delta_{bo}$이다.
...
\caption{20-인스턴스 test 결과(실험 조건당 인스턴스 20개 $\times$ replication 5회).
$\Delta_{bo}$는 관측된 최선해 대비 평균 gap, 시간은 1회 실행 평균이다. vs CP-SAT는
CP-SAT가 기준해를 제공한 부분집합(S 조건 각 5개, M-none 2개)에서만 계산하였으며,
이 가운데 S-none만 최적성이 증명되었다. 같은 부분집합에서 CP-SAT의 평균 실행시간은
S 조건 76초, M-none 237초이다.}
\label{tab:main}\small
\begin{tabular}{@{}llrrrr@{}}
\toprule
실험 조건 & 방법 & 평균목적값$\pm$표준편차 & $\Delta_{bo}$(\%) & vs CP-SAT(\%) & 시간(초)\\
\midrule
S-none & VAA & $1497.5\pm400.3$ & 6.60 & $+6.49$ & 0.00\\
 & Paper-SA-RL5 & $1423.3\pm383.6$ & 1.03 & $+1.21$ & 0.15\\
 & GILS-uniform & $1411.8\pm382.7$ & 0.16 & $+0.21$ & 0.08\\
 & GILS-tabular & $1412.9\pm381.3$ & 0.28 & $+0.33$ & 0.11\\
 & GILS-DQN & $1416.1\pm382.4$ & 0.51 & $+0.75$ & 0.23\\
S-medium & VAA & $5447.5\pm1468.9$ & 3.89 & $+3.33$ & 0.00\\
 & GILS-uniform & $5255.7\pm1413.3$ & 0.08 & $+0.11$ & 0.08\\
 & GILS-tabular & $5259.1\pm1412.3$ & 0.15 & $+0.18$ & 0.11\\
 & GILS-DQN & $5270.8\pm1415.3$ & 0.38 & $+0.55$ & 0.22\\
S-tight & VAA & $9276.8\pm2387.6$ & 2.90 & $+2.17$ & 0.00\\
 & GILS-uniform & $9010.8\pm2206.1$ & 0.09 & $+0.21$ & 0.09\\
 & GILS-tabular & $9011.9\pm2206.7$ & 0.11 & $+0.17$ & 0.11\\
 & GILS-DQN & $9022.0\pm2205.3$ & 0.23 & $+0.34$ & 0.22\\
M-none & VAA & $3030.9\pm903.6$ & 3.73 & $+4.80$ & 0.00\\
 & Paper-SA-RL5 & $2976.4\pm864.4$ & 1.86 & $+0.99$ & 0.55\\
 & GILS-uniform & $2928.9\pm842.6$ & 0.29 & $+0.13$ & 0.43\\
 & GILS-tabular & $2932.1\pm846.3$ & 0.37 & $+0.11$ & 0.52\\
 & GILS-DQN & $2932.5\pm845.5$ & 0.40 & $+0.19$ & 0.73\\
M-medium & VAA & $23025.5\pm4075.4$ & 2.56 & --- & 0.00\\
 & GILS-uniform & $22526.4\pm3960.5$ & 0.29 & --- & 0.44\\
 & GILS-tabular & $22515.5\pm3964.0$ & 0.24 & --- & 0.55\\
 & GILS-DQN & $22540.3\pm3956.4$ & 0.36 & --- & 0.75\\
M-tight & VAA & $38903.7\pm11283.5$ & 1.25 & --- & 0.01\\
 & GILS-uniform & $38528.0\pm11078.0$ & 0.19 & --- & 0.48\\
 & GILS-tabular & $38512.1\pm11059.5$ & 0.16 & --- & 0.60\\
 & GILS-DQN & $38543.6\pm11077.6$ & 0.24 & --- & 0.81\\
L-none & VAA & $5492.3\pm1482.2$ & 2.21 & --- & 0.02\\
 & Paper-SA-RL5 & $5479.7\pm1449.5$ & 1.97 & --- & 1.92\\
 & GILS-uniform & $5388.4\pm1427.3$ & 0.27 & --- & 2.16\\
 & GILS-tabular & $5385.5\pm1424.6$ & 0.22 & --- & 2.54\\
 & GILS-DQN & $5386.1\pm1425.2$ & 0.23 & --- & 2.99\\
L-medium & VAA & $55611.1\pm18087.3$ & 1.52 & --- & 0.02\\
 & GILS-uniform & $54846.4\pm17523.5$ & 0.10 & --- & 2.10\\
 & GILS-tabular & $54833.4\pm17526.8$ & 0.07 & --- & 2.52\\
 & GILS-DQN & $54848.8\pm17522.9$ & 0.11 & --- & 2.85\\
L-tight & VAA & $119671.9\pm32475.3$ & 0.77 & --- & 0.02\\
 & GILS-uniform & $118901.4\pm31781.1$ & 0.08 & --- & 2.01\\
 & GILS-tabular & $118865.6\pm31788.4$ & 0.05 & --- & 2.51\\
 & GILS-DQN & $118888.8\pm31789.1$ & 0.07 & --- & 2.76\\
\bottomrule
\end{tabular}
\end{table}

\subsection{핵심 구성요소 분석: 성능은 어디에서 오는가}
...
\begin{table}[H]\centering
\caption{Operator-pool ablation(실험 조건당 인스턴스 5개 $\times$ replication 5회, 반복 횟수 1{,}000회 고정; 양측 Wilcoxon, $+$평균 = 더 풍부한 연산자 집합이 우세).}
\label{tab:b1}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
대비 & $n$ & 개선(\%p) & $p$\\
...
\caption{연산자 집합별 관측된 최선해 대비 gap.}\label{fig:op}
\end{figure}
...
\begin{table}[H]\centering
\caption{Engine-component ablation(실험 조건당 인스턴스 5개 $\times$ replication 5회;
leave-one-out 방식, 제거 시 상대 목적값 악화율).}
\label{tab:b2}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
제거 구성요소 & $n$ & 악화(\%p) & $p$\\
...
\caption{구성요소 제거 시 목적값 악화율.}\label{fig:comp}
\end{figure}

\subsection{보조 분석: 학습 기반 연산자 선택}
연산자 집합과 구성요소를 full로 고정하고 selection만 바꾸면(표~\ref{tab:sel}), 어떤
learned policy도 uniform을 실용적으로 능가하지 못한다. 반복 횟수를 50--3{,}000회로 바꾸어도 결과는 같고 우열의 방향이
반복 횟수에 따라 뒤집히며(부록 그림~\ref{fig:sel}),
이러한 부정적 결과는 무제약 조건만으로도 성립하므로 시간 제약 확장 때문에 나타난
결과가 아니다.

\begin{table}[H]\centering
\caption{selection 효과(실험 조건당 인스턴스 5개 $\times$ replication 5회, 반복 횟수 1{,}000회;
양측 Wilcoxon, 양의 평균 = 앞 방법 우세).}
\label{tab:sel}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
비교 & 평균차(\%p) & $p$ & 판정\\
...
식~\eqref{eq:obj}이 시간 단위 성분 둘을 합치므로 tuning 집합에서
...
\section{논의}
학습 기반 selector가 uniform 대비 이득을 주지 못한 것은 학습 일반에 대한 결론이
...
흡수한다. 반복 횟수별 gap 곡선도 대부분의 test 인스턴스에서 해 품질이 1{,}000회 이전에
평탄해짐을 보인다. SA acceptance와 reheating은 원 모델과 비교할 수 있도록
유지하였으나, ablation은 본 설정에서 이 둘이 해 품질에 필수적이지 않음을 보인다.

selector가 예측할 대상이 마땅치 않다는 점도 작용한다. 본 문제의 인스턴스는 정적이고
모든 release time이 최적화 이전에 알려져 있어, 실행 도중 새로 드러나 정책이 활용할
정보가 없다. 도착 정보가 시간에 따라 공개되는 동적 도크 운영이나 이동 하나의 평가가
비싼 시뮬레이션 기반 모형에서는 판단이 달라질 수 있다. 따라서 본 결과의 함의는 학습이
무용하다는 것이 아니라 비교 조건에 있다. learned selector는 동일한 엔진과 동일한
반복 횟수 아래 uniform sampling과 견주어야 하며, 그 통제가 없으면 descent·restart·강한
연산자 집합의 이득이 정책의 성과로 잘못 귀속될 수 있다.

\section{결론}
본 연구는 부분 하역 병행 트럭 스케줄링에 release time과 soft due date를
...
\caption{반복 횟수별 관측된 최선해 대비 gap. 50--3{,}000회에서 uniform은
$0.47\to0.16\%$, tabular는 $0.56\to0.09\%$, DQN은 $0.56\to0.26\%$로 움직이며,
어떤 반복 횟수에서도 learned selector가 uniform을 뚜렷하게 능가하지 않는다.}\label{fig:sel}
\end{figure}
...
\bibitem{assadi2016} M. T. Assadi and M. Bagheri, ``Differential evolution and population-based simulated annealing for truck scheduling problem in multiple door cross-docking systems,'' \emph{Computers \& Industrial Engineering}, vol. 96, pp. 149--161, 2016.
...
\bibitem{li2026} Y. Li, M. Mohammadi, X. Zhang, Y. Lan, and W. van Jaarsveld, ``Integrated trucks assignment and scheduling problem with mixed service mode docks: A Q-learning based adaptive large neighborhood search algorithm,'' \emph{European Journal of Operational Research}, vol. 333, no. 1, pp. 117--137, 2026.
...
\bibitem{rijal2019} A. Rijal, M. Bijvank, and R. de Koster, ``Integrated scheduling and assignment of trucks at unit-load cross-dock terminals with mixed service mode dock doors,'' \emph{European Journal of Operational Research}, vol. 278, no. 3, pp. 752--771, 2019.
...
\bibitem{xi2020} X. Xi, C. Liu, Y. Wang, and L. H. Lee, ``Two-stage conflict robust optimization models for cross-dock truck scheduling problem under uncertainty,'' \emph{Transportation Research Part E}, vol. 144, Art. 102123, 2020.
\end{thebibliography}
