...
총 작업완료시간을 크게 줄이는 것으로 보고되어 LTL(Less-Than-Truckload, 소량 화물 혼적) 통합 네트워크에서 실용적 매력이 크다.
...
이송 화물에 의존하는 모든 출고 운송 트럭으로 전파되고, 연성 납기는 이 연쇄 지연을
...
도착 시각과 연성 납기를 갖는 문제로 확장하고, 이를 독립적인 스케줄 평가기와
...
\includegraphics[width=0.92\columnwidth]{{fig0_compound_timeline.drawio}.png}
...
$\lambda$는 납기지연 한 단위 시간에 부과하는 벌점 계수(penalty coefficient)로, 작업완료시간과 납기 준수 사이의
관리적 선호를 반영하는 파라미터이다(6.4절). $r_q=0$, $\bar d_q=\infty$이면 기존 총 작업완료시간 모델로 환원된다. CP-SAT는 incumbent와
...
incumbent를 갱신한다. acceptance는 후보가 현재 해를 엄격히 개선할 때만 받아들이는
greedy 규칙이다. 이 규칙은 조정 집합에서 simulated annealing(Kirkpatrick 등, 1983) acceptance(초기 온도
$T_0=\max\{1,0.05\,J(s_0)\}$, 기하 냉각 $T\leftarrow0.995\,T$, 재시작 시 reheating)와 비교해
정하였다. 조정 집합 45개 인스턴스 중 33개에서 SA acceptance의 목적값이 greedy보다 높았고(인스턴스별
...
restart를 더하며, 확률적 acceptance 대신 greedy acceptance를 쓴다. ablation(6.2절)은 이 가운데 kick restart의
기여가 가장 크고 guided 연산자와 descent가 그 뒤를 이음을 보인다. 두 방법의 구조적 차이는
...
선택·SA acceptance를 그대로 두고 evaluator와 목적식만 식 (16)으로 확장한 것이며,
...
집합에서만 훈련하고 $\lambda$와 acceptance 규칙 같은 설계 값은 조정 집합에서만 고르므로 시험 집합의
...
CP-SAT가 제한시간 안에 반환한 incumbent로, 최적성이 증명되지 않은 해를 포함하며 "최적"으로 부르지 않는다.
...
한 반복은 선택된 연산자 하나로 후보해를 생성·평가하는 단계이며, GILS는 여기에 개선 시 descent를 더 수행한다.
...
증명된다. 보고하는 CP-SAT 계산 시간은 모형 구성과 solver 종료 처리를 포함한 wall-clock 시간이므로
...
평균 0.06--0.25\%이다(학습 선택 정책을 포함한 전체 변형은 0.06--0.47\%). 같은 선택 정책 기준으로 VAA 대비
...
CP-SAT가 기준해를 반환한 조건에서 CP-SAT의 평균 계산 시간은 S 76초, M-none 606초로 같은 조건의 GILS(0.09, 0.48초)보다
100배 이상 길며, M의 시간 제약 조건과 L의 모든 조건에서는 CP-SAT가 제한시간 안에 실행 가능해를 반환하지 못했다.

GILS는 개선 이동 뒤 descent를 추가로 수행하므로 같은 반복 횟수가 같은 계산량을 뜻하지는 않는다. 이를 통제하기
위해, 각 시험 실행(조건·인스턴스·반복)에서 GILS가 실제로 사용한 실행 시간을 같은 실행의 SA-RL5(시간 제약 조건은
Extended SA-RL5)에 그대로 부여하고 시간이 다할 때까지 반복하게 한 동일 wall-clock 비교를 추가로 수행하였다.
이때 SA-RL5의 평균 반복 횟수는 S/M/L에서 각각 586, 1{,}102, 2{,}020회로, L에서는 반복 1{,}000회 비교보다 약 2배의
탐색을 수행한다(S에서는 GILS가 더 빨라 1{,}000회보다 적다). 그럼에도 GILS는 동일 시간의 SA-RL5보다 평균
1.08\%(S 1.23\%, M 1.24\%, L 0.76\%; $n=180$, $p<0.0001$) 좋은 해를 찾았고, 동일 시간의 SA-RL5가 GILS보다 좋은
인스턴스는 180개 중 하나도 없었다. 따라서 GILS의 성능 향상은 추가 계산량이 아니라 탐색 구조에서 비롯된다.

\begin{table}[H]\centering
\caption{Table 3. Test results over 20 instances per experimental condition (20 instances $\times$ 5 replications). $\Delta_{bo}$ is the mean gap to the best observed solution and time is the mean per run. The vs CP-SAT column is computed only on the subset for which CP-SAT returned a reference (5 instances per S condition, 2 for M-none); optimality was proven only for S-none. On that subset the mean CP-SAT runtime is 76 s (S) and 606 s (M-none). Runtimes are wall-clock times including model construction and solver termination overhead and may therefore slightly exceed the nominal 600-s solver limit.}
\label{tab:main}\small
\begin{tabular}{@{}llrrrr@{}}
\toprule
Condition & Method & Mean obj. ± s.d. & Δbo(\%) & vs CP-SAT(\%) & Time(s)\\
\midrule
S-none & VAA & 1497.5 ± 400.3 & 6.58 & +6.49 & 0.00\\
 & Paper-SA-RL5 & 1423.3 ± 383.6 & 1.01 & +1.21 & 0.15\\
 & GILS-uniform & 1412.5 ± 381.8 & 0.22 & +0.23 & 0.09\\
 & GILS-tabular & 1413.7 ± 382.5 & 0.28 & +0.42 & 0.11\\
 & GILS-DQN & 1413.4 ± 382.1 & 0.27 & +0.40 & 0.23\\
S-medium & VAA & 5447.5 ± 1468.9 & 3.87 & +3.33 & 0.00\\
 & Extended-SA-RL5 & 5289.1 ± 1428.9 & 0.69 & +0.90 & 0.17\\
 & GILS-uniform & 5258.8 ± 1412.7 & 0.13 & +0.16 & 0.09\\
 & GILS-tabular & 5262.0 ± 1409.6 & 0.21 & +0.17 & 0.12\\
 & GILS-DQN & 5275.2 ± 1412.5 & 0.47 & +0.47 & 0.23\\
S-tight & VAA & 9276.8 ± 2387.6 & 2.90 & +2.17 & 0.00\\
 & Extended-SA-RL5 & 9057.4 ± 2224.3 & 0.59 & +0.69 & 0.17\\
 & GILS-uniform & 9012.8 ± 2205.5 & 0.11 & +0.25 & 0.09\\
 & GILS-tabular & 9016.9 ± 2206.4 & 0.16 & +0.30 & 0.12\\
 & GILS-DQN & 9024.3 ± 2209.0 & 0.24 & +0.43 & 0.24\\
...
\textbf{Engine components.} 연산자 집합을 전체로, 선택 정책을 균등 선택으로 고정한 최종 엔진에서
구성요소를 하나씩 제거하거나 SA acceptance와 reheating을 추가하면(<Table~\ref{tab:b2}>,
<Figure~\ref{fig:comp}>) kick restart 제거에 따른 악화가 가장 크고 시간 제약 유무와 관계없이 유의하다.
descent 제거와 무작위 초기해의 악화도 유의하지만 그 크기는 restart 제거의 약 1/9 수준이며, 무작위
초기해는 시간 제약 유무로 나누면 유의하지 않다. SA acceptance와 reheating을 추가하면 목적값이 오히려
나빠져, 조정 집합에서 정한 greedy acceptance를 시험 집합에서도 뒷받침한다.

\begin{table}[H]\centering
\caption{Table 5. Engine-component ablation (5 instances $\times$ 5 replications per condition over 9 conditions; one change at a time to the final greedy-acceptance engine, relative objective degradation after the change; two-sided Wilcoxon on per-instance replication means, hence at most 45 pairs, with zero differences excluded from $n$).}
\label{tab:b2}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
...
\begin{table}[H]\centering
\caption{Table 6. Effect of the selection policy (20 instances $\times$ 5 replications per condition over 9 conditions, 1,000 iterations; two-sided Wilcoxon on per-instance replication means, hence at most 180 pairs, with zero differences excluded from $n$; positive mean favors the first method).}
\label{tab:sel}\small
\begin{tabular}{@{}lrrrl@{}}
\toprule
...
같은 것은 아니다. $\lambda$는 운영자가 작업완료시간 대비 납기 준수를 얼마나 중시하는지를 나타내는
...
정체 탈출은 kick restart가 맡으므로 SA acceptance와 reheating의 역할은
이와 겹치며, 조정 집합 비교와 시험 집합 ablation에서 이 둘이 해 품질을 높이지 않아 최종
VAA-GILS에서 제외하였다(4장, <Table~\ref{tab:b2}>). 즉 본 문제에서 성능의 본질은 학습된 선택 정책이나
확률적 acceptance가 아니라 kick restart, bottleneck-guided 연산자, 결정론적 descent에 있다.
...
\section{8. 결 론}
본 연구는 부분 하역 병행 트럭 스케줄링에 도착 시각과 연성 납기를
도입하고, MILP·CP-SAT 정식화와 유효 하한으로 휴리스틱 해를 검증하였다. CP-SAT
기준해가 있는 실험 조건에서 VAA-GILS의 목적값은 해당 기준해 대비 평균 0.06--0.25\%였고(소형 무제약
조건에서는 CP-SAT가 최적성을 증명하였다), CP-SAT가 600초 안에 실행 가능해를 찾지 못한 대형 시간 제약
조건에서도 약 3초 안에 VAA와 Extended SA-RL5보다 좋은 해를 제공하였다.
ablation은 성능이 kick restart, guided 이동, best-improvement descent라는 결정론적
구조에서 비롯되며, 학습 기반 연산자 선택과 확률적 acceptance는 이
환경에서 실용적 이득을 주지 않아 최종 알고리즘은 greedy acceptance를 사용한다. 향후에는 도착 정보가 실시간으로 공개되는 동적
