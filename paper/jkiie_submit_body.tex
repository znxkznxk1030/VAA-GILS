...
이것이 품질 이득의 대부분을 설명함을 보인다. Table~\ref{tab:base}가 두 방법의 구조적
차이를 요약한다.
...
seed는 서로소인 train/tuning/test 집합으로 분리한다. 주 해품질 비교(Table~\ref{tab:main})는
...
\section{6. 실험 결과}
\subsection{6.1 해 품질}
Table~\ref{tab:main}이 20-인스턴스 test 결과를 요약한다. CP-SAT가 기준해를 제공한
...
generic $\ge$ critical $\ge$ full로 단조 감소한다(Figure~\ref{fig:op}). 짝지은
Wilcoxon(Table~\ref{tab:b1})은 유도 연산자가 목적값을 일관되게·유의하게 낮춤을
보인다.

\begin{table}[H]\centering
\caption{Table 3. Operator-pool ablation (5 instances $\times$ 5 replications per condition, budget fixed at 1,000 iterations; two-sided Wilcoxon, positive mean favors the richer operator set).}
\label{tab:b1}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
Contrast & n & Gain(\%p) & p\\
...
\caption{Figure 1. Mean gap to the best observed solution by operator set.}\label{fig:op}
\end{figure}
...
\begin{table}[H]\centering
\caption{Table 4. Engine-component ablation (5 instances $\times$ 5 replications per condition; leave-one-out, relative objective degradation when a component is removed).}
\label{tab:b2}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
...
\caption{Figure 2. Objective degradation when each component is removed.}\label{fig:comp}
\end{figure}

\subsection{6.3 보조 분석: 학습 기반 연산자 선택}
연산자 집합과 구성요소를 full로 고정하고 selection만 바꾸면(Table~\ref{tab:sel}), 어떤
learned policy도 uniform을 실용적으로 능가하지 못한다. 50--3{,}000회의 모든 예산
구간에서도 결과는 같고 우열의 방향이 예산에 따라 뒤집히며(Figure~\ref{fig:sel}),
이러한 부정적 결과는 무제약 조건만으로도 성립하므로 시간 제약 확장 때문에 나타난
결과가 아니다.

\begin{table}[H]\centering
\caption{Table 5. Effect of the selection policy (5 instances $\times$ 5 replications per condition, 1,000-iteration budget; two-sided Wilcoxon, positive mean favors the first method).}
\label{tab:sel}\small
\begin{tabular}{@{}lrrl@{}}
\toprule
...
인스턴스별로 min--max 정규화해 평균한다(Figure~\ref{fig:lam}). tardiness 감소의
...
\caption{Figure 3. Normalized makespan-tardiness trade-off on tuning instances.}\label{fig:lam}
\end{figure}
...
\caption{Figure 4. Mean gap to the best observed solution across iteration budgets.}\label{fig:sel}
\end{figure}
